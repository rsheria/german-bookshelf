...
-- Fixes both IP logging and last_active timestamp updating with proper function drops
...
DO $$
BEGIN
  -- Drop record_user_ip function with all possible signatures
  BEGIN
    DROP FUNCTION IF EXISTS public.record_user_ip(uuid, text, text, text);
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Drop function 1 failed: %', SQLERRM;
  END;
  
  BEGIN
    DROP FUNCTION IF EXISTS public.record_user_ip(uuid, text, text);
  EXCEPTION WHEN OTHERS THEN
...
-- Drop existing update_last_active functions
DO $$
BEGIN
  BEGIN
    DROP FUNCTION IF EXISTS public.update_last_active_on_ip_log();
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Drop function 4 failed: %', SQLERRM;
  END;
  
  BEGIN
    DROP TRIGGER IF EXISTS update_last_active_trigger ON public.ip_logs;
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Drop trigger failed: %', SQLERRM;
  END;
END $$;
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Drop function 5 failed: %', SQLERRM;
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Drop function 6 failed: %', SQLERRM;
  END;
  
  BEGIN
    DROP FUNCTION IF EXISTS public.admin_get_profiles;
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Drop function 7 failed: %', SQLERRM;
  END;
END $$;
...
BEGIN
  BEGIN
    DROP FUNCTION IF EXISTS public.manual_update_last_active(uuid);
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Drop function 8 failed: %', SQLERRM;
  END;
END $$;

CREATE FUNCTION public.manual_update_last_active(user_id_param uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
